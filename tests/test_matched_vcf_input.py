...
class TestClass():
  ...
  #organoid check
  muts_vcf = test_out + 'input.muts.vcf.gz'
  lof_vcf = test_out + 'input.genes.lof.vcf.gz'
  drv_vcf = test_out + 'input.drv.vcf.gz'

  my_formatter=formatter.IO_Formatter(**options_vcf) 
  outdir_path=my_formatter.format(['outdir'])

  def test_matched_vcf_input(self):
  ...
  def test_matched_vcf_formatter(self):
    f = self.my_formatter
    outdir_path = self.outdir_path
    with formatter.tempdir(outdir_path['outdir']) as base_dir:
      obs_muts_vcf = base_dir + '/input_muts.vcf.gz'
      obs_lof_vcf = base_dir + '/input_genes_lof.vcf.gz'
      obs_drv_vcf = base_dir + '/input_drv.vcf.gz'
