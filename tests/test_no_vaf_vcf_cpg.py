...
class TestClass():
  ...
  test_dir = configdir + '/test_input_no_vaf_vcf_cpg/'
  test_out = configdir + '/test_output_no_vaf_vcf_cpg/'
  ...
  options_vcf = {'vcf_file': test_dir + 'input.vcf.gz',
                   ...
                   'cancer_predisposition': test_dir + 'cpg_variants_sorted.tsv.gz',
  ...
  file_dict = {'input_status': {'mutations': True, 'lof_genes': True,'cancer_predisposition':True,
  ...
  cpv_vcf = f"{test_out}/input_cpv.vcf.gz"
  ...
  def test_no_VAF_vcf_formatter(self):
    ...
    with formatter.tempdir(outdir_path['outdir']) as base_dir:
      ...
      obs_cpv_vcf = f"{base_dir}/input_cpv.vcf.gz"
      ...
      exp_cpv_vcf_sub = annotator.unheader_vcf(self.cpv_vcf, base_dir+'/exp_cpv.vcf')
      obs_cpv_vcf_sub = annotator.unheader_vcf(obs_cpv_vcf, base_dir+'/obs_cpv.vcf')
      assert filecmp.cmp(exp_cpv_vcf_sub, obs_cpv_vcf_sub, shallow=True), 'cpv records in vcf files are identical OK'
